    ...
    public function execute()
    {
        ...
        if ($id) {
            try {
                ...
                $model->load($id);
                $model->delete();
                $this->messageManager->addSuccess(__('The sponsers has been deleted.'));
                return $resultRedirect->setPath('*/*/');
            } catch (\Exception $e) {
                $this->messageManager->addError ($e->getMessage());
                return $resultRedirect->setPath('*/*/edit', ['sponsors_id' => $id]);
            }
        }
        $this->messageManager->addError(__('We can\'t find a sponsers to delete.'));
        return $resultRedirect->setPath('*/*/');
    }
}